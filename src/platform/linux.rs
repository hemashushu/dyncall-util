...
mod tests {
    ...
    fn test_load_library() {
        // note:
        //
        // make sure the specified version of libc exists first:
    ...
    fn test_load_user_library() {
        ...
        // note:
        // CARGO_PKG_NAME = "dyncall-util"
        // CARGO_CRATE_NAME = "dyncall_util"

        let mut pwd = std::env::current_dir().unwrap();

        // in the VSCode editor `Debug` environment,
        // the function `current_dir()` returns the project's root folder.
        // while in both command `$ cargo test` and VSCode editor's `Run Test` cases,
        // the function `current_dir()` returns the current crate's path instead.
        // the following is used to normalize the path to test resources when
        // the current crate is a member of the workspace.
        //
